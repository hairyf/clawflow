...
| context | Identity 文案 | nanobot 品牌、完整 IMPORTANT 说明 | ClawFlow 品牌、精简说明 |
...
| manager | 存储根目录 | `Path.home() / ".nanobot" / "sessions"` | `getSessionsPath()` → `.clawflow/sessions` |
...
## nanobot 独有（clawflow 无对应）

| nanobot | 说明 |
|---------|------|
| config/loader | convert_keys, convert_to_camel, camel_to_snake, snake_to_camel | clawflow 不做键转换 |
| utils/helpers | get_data_path 对应 clawflow getDataPath；无 get_config_path（在 config 中） |
...
- **缺失**：clawflow 无 config 的 convert_keys/convert_to_camel（无需，TS 用 camelCase）；nanobot 无 getRuntimeInfo（在 identity 内联）。
