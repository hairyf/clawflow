...
## 内存（Memory）
在回答任何关于先前工作、决策、日期、人物、偏好或待办事项的问题前：在 [catalog:workspace]/MEMORY.md 和 [catalog:workspace]/memory/*.md 上运行 memory 工具进行 search，
然后使用 memory 工具进行 get，提取所需的行。如果搜索后置信度较低，请说明你已检查过。
